...
reconstruct_signal = sum(dem_signals)   # Superimpose to obtain the reconstructed signal 叠加得到重构信号

# Play the audio 播放音频
volume = 0.001
play_signal = reconstruct_signal * volume   # Prevent the audio too loud 防止音频响度过大
sd.play(play_signal, sample_rate)
sd.wait()
print("The text in the audio is : \" Correct \" ")

# Draw the demodulated signal 绘制解调信号
plt.figure(2)
plt.plot(time, reconstruct_signal)
